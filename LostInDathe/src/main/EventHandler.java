...
public class EventHandler {
	...
	public void checkEvent() {
		...
		if(canTouchEvent == true) {
				...
				else if(hit(2, 29, 18, "any") == true) { interTeleport(1, 16, 15); gp.stopMusic(49); gp.playMusic(2); }
				// Teleport OG -> Chemie: Chemie -> OG
				else if(hit(1, 72, 41, "any") == true) { interTeleport(4, 53, 63.9); gp.stopMusic(2); gp.playMusic(5); }
				else if(hit(4, 53, 64, "any") == true) { interTeleport(1, 72, 40.9); gp.stopMusic(5); gp.playMusic(2); }

				else if(hit(1, 72, 34, "any") == true) { interTeleport(4, 53, 70.9); gp.stopMusic(2); gp.playMusic(5); }
				else if(hit(4, 53, 71, "any") == true) { interTeleport(1, 72, 33.9); gp.stopMusic(5); gp.playMusic(2); }

				else if(hit(1, 72, 55, "any") == true) { interTeleport(4, 53, 52.9); gp.stopMusic(2); gp.playMusic(5); }
				else if(hit(4, 53, 53, "any") == true) { interTeleport(1, 72, 54.9); gp.stopMusic(5); gp.playMusic(2); }
				// Teleport 2x EG -> Info; Info -> OG
				else if(hit(5, 42, 24, "any") == true) { interTeleport(3, 50, 49.9); gp.stopMusic(2); gp.playMusic(48); keyH.licht = false; }
